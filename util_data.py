...
def get_dataset(datatype, n_client, n_c, alpha, partition_equal=True):
    ...
    if datatype == 'CIFAR10' or datatype == 'CIFAR100' or datatype == 'MNIST' or datatype == 'FashionMNIST' or datatype == 'CINIC10':
        ...
        if datatype == 'CINIC10':
            ...
            dataset_train_global = datasets.ImageFolder('cinic_train', transform=transforms.Compose(
                [transforms.ToTensor(), transforms.Normalize(mean=cinic_mean, std=cinic_std)]))
            dataset_test_global = datasets.ImageFolder('cinic_test', transform=transforms.Compose(
                [transforms.ToTensor(), transforms.Normalize(mean=cinic_mean, std=cinic_std)]))
